...
class LawsSearch extends Component {
  ...
  LawSection_searchActs(query) {
    ...
    act.forEach(index => {
      ...
      actArray.push(Global_Attributes.LawSection['actno'][i] + " : " + Global_Attributes.LawSection['acttitle'][i]);
    });
  ...
  setAct = (item) => {
    ...
    Global_Attributes.PonOneBean['fine'] = Global_Attributes.PonLaws['set_fine'][index];
    Global_Attributes.PonOneBean['payable'] = Global_Attributes.PonLaws['total_payable'][index];
    Global_Attributes.PonOneBean['sect'] = Global_Attributes.PonLaws['act_no'][index];
    Global_Attributes.PonOneBean['didCommit'] = Global_Attributes.PonLaws['act_title'][index];
  ...
  handleDIDCommitSearch = (text) => {
    if (text.length > 1) {
      this.setState({ query: text, acttitlevalue: text, isDIDCOMMITPopoverVisible:true,});
      Global_Attributes.PonOneBean['didCommit'] = text;
      console.log("hii")
    }
    else {
      this.setState({ query: text, acttitlevalue: text, isDIDCOMMITPopoverVisible: false });
      Global_Attributes.PonOneBean['didCommit'] = text
    }
  }
  ...
  handleSECTSearch = (text) => {
    if (text) {
      this.setState({ query1: text, actnovalue: text, isSECTPopoverVisible:true });
      Global_Attributes.PonOneBean['sect'] = text;
    }
    else {
      this.setState({ query: text, actnovalue: text, isSECTPopoverVisible: false });
      Global_Attributes.PonOneBean['sect'] = text
    }
  }
          ...
          <>
            <View style={LawsParentTitleStyle.searchSection}>
              ...
              <Autocomplete
                ...
                onChangeText={text => { this.handleLawSectionSearch(text) }}
            ...
            <Modal
              ...
              animationDuration={0}
              onHardwareBackPress={() => { this.setState({ LawSectionfocus: true, modalVisible: false }) }}
              onShow={() => { this.setState({ LawSectionfocus: true }) }}
              onModalHide={() => { this.setState({ LawSectionfocus: true, modalVisible: false }) }}
                      ...
                      <View>
                        <TouchableOpacity
                          ...
                          <Image source={require('../assets/search.png')}
                            style={{ width: 20, height: 20, tintColor: 'grey', position: 'absolute', right: 5, top: 10 }}></Image>
                        </TouchableOpacity>
                      </View>
                    );
                  }}
          ...
          <View>
            <PopoverContainer padding={20}>
              <Popover 
                ...
                component={()=>(
                  <View style={act_title != '' ? styles.DidCommitFlatList : { flex: 1 }}>
                  <FlatList
                    keyboardShouldPersistTaps={'handled'}
                    ...
                    renderItem={({ item }) => {
                      return (
                        <View style={{ marginLeft: 10, marginRight: 10 }}>
                          <TouchableOpacity
                            onPress={() => { this.setState({ acttitlevalue: item, query: '', actnovalue: this.setAct(item), isDIDCOMMITPopoverVisible: false }) }}>
                            <Text style={styles.itemText}>{item}</Text>
                          </TouchableOpacity>
                        </View>
                      );
              ...
              <View style={styles.autocompleteView}>
                <Autocomplete
                  ...
                  onChangeText={text => {this.handleDIDCommitSearch(text)}}
                  ...
                  renderItem={({ item }) => {
                    return (
                      <View style={{ display: 'flex', flexDirection: 'row', width: '90%'}}>
                        <TouchableOpacity
                          style={{ width: "95%" }}
                          onPress={() => { this.setState({ actnovalue: (item.match(/.*(?=\s:)/))[0], query1: '', acttitlevalue: this.setActNo(item),isSECTPopoverVisible:false })}}>
                          <Text style={styles.itemText}>{item}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={{ width: "5%" }}
                          onPress={() => this.PonOffence_lawDescription(item)}>
                          <Image source={Viewdetails} style={{ height: 35, width: 35, tintColor: 'red' }} />
                        </TouchableOpacity>
                      </View>
                    );
                  }}
          ...
          >
           <View style={styles.autocompleteView}>
              <Autocomplete
                ...
                placeholder="SECT*"
                placeholderTextColor='grey'
                onChangeText={text => { this.handleSECTSearch(text) }}
