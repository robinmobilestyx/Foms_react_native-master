...
import { View, Text,StatusBar,TouchableOpacity,Image,StyleSheet, ScrollView,FlatList } from 'react-native';
...
class LawsActTitle extends React.Component {
  ...
  lawsDescription=(no,title,pay,des,fine,demerits)=>{
   ...
   {actno:actno,acttitle:acttitle,payment:payment,description:description,set_fine:set_fine,demerit_points:demerit_points});
  }
    ...
    //  console.log('length is:-'+Length)
     for (i= 0; i <Length; i++) 
     {
       ...
       var tempItem=  (
             <View key={i} >
                 <TouchableOpacity 
                onPress={()=>{this.lawsDescription(no,title,pay,des,fine,demerits)}}
