...
import { SafeAreaView, StyleSheet, View, Image, Text, Platform, Alert, Keyboard } from 'react-native';
...
const New_DL_CustomerFirstName = "DAC", New_DL_CustomerMiddleName = "DAD", Customer_Family_Name = "DCS",
    ...
    Date_Of_Birth = "DBB", Sex = "DBC", Issue_Date = "DBD";

var allKeys = [New_DL_CustomerFirstName, New_DL_CustomerMiddleName, Customer_Family_Name, Street_Address_1, Customer_Given_Name
    , Name_Suffix, City, Jurisdction_Code, Postal_Code, Country_Identification, Customer_Id_Number, Class, Restrictions, Date_Of_Birth
    , Sex, Issue_Date];
...
export default class PonInfo extends Component {
    ...
    discoverDeviceHandler = (flash) => {
         Scanner.callDeviceDiscovery((flash),
         (sucess) =>{
           ...
           // this.parsScanData(sucess);
            const lines  = sucess.split(", ");  //\\r?\\n
                        ...
                        if(values.length > 3 ||  values.substring(3).localeCompare("NONE")== -1){
          ...
          if(myData[Date_Of_Birth] != undefined)
          {
            ...
            let formate = myData[Date_Of_Birth].slice(0,4)+ '-' +myData[Date_Of_Birth].slice(4,6)+'-'+myData[Date_Of_Birth].slice(6,8)  
            let ld = new Date(formate) 
            console.log(ld.toDateString()) 
            console.log('lic',ld.getFullYear(),'/',ld.getMonth()+1,'/',ld.getDate());
           // const formatedDob  = new Date(ld.getFullYear()+"/"+ld.getMonth()+1+"/"+ld.getDate())
            this.setState({bd:ld});
          }
         ...
         // const family = myData[Customer_Family_Name].split(",");
        //   var name = myData[Customer_Given_Name].toString();
        //   const fullName = name.split(",");
        //   const given = fullName[0];
        //   const initials = fullName[1];
         // console.log("dob"+myData[Customer_Family_Name])
                                   // this.setState({family:family[0]});
                                    // this.setState({given:given});
                                    // this.setState({initials:myData[Customer_Given_Name].charAt(0)});
                                    // Global_Attributes.PonOneBean['family'] = myData[Customer_Family_Name];
                                    // Global_Attributes.PonOneBean['given'] = myData[Customer_Given_Name];
                                    // Global_Attributes.PonOneBean['initials'] = myData[Customer_Family_Name].charAt(0);
    ...
    //                 if(allKeys.includes(key)){
    //                     if(value.localeCompare("NONE")== -1){
    //                         const keys = allKeys.findIndex(data => data.indexOf(key));
    //                         myData[keys] = value;
    //                         console.log("myData values:"+keys+"="+value)
    //                     }
    //                 }
    ...
    componentDidMount() {
       ...
       // alert(getTime);
        ...
        new LawsController().getAllActs(All_acts, this.props, "PonInfo", Global_Attributes.laws);
    }
    ...
    onDateTimeChange = (_event, selectedDate) => {
        ...
        let storeDate = currentDate.getFullYear() + "/" + (currentDate.getMonth() + 1) + "/" + currentDate.getDate();
        // alert(storeDate);
    ...
    dobChange = (_event, selectedDate) => {
        ...
        let BdDate = selectedDate.getFullYear() + "/" + (selectedDate.getMonth() + 1) + "/" + selectedDate.getDate();
        // alert(BdDate);
    ...
    render() {
        ...
        return (
            <SafeAreaView style={ponstyle.mainView} >
                <ScrollView style={ponstyle.Scrollview}>

                    <View style={ponstyle.locationView}>

                        <View style={ponstyle.loc_code_view}>
                            <Text style={ponstyle.loc_code_text}>Location Code</Text>
                            <Text style={ponstyle.loc_code_no}>{Global_Attributes.PonOneBean['locationCode']}</Text>
                        </View>
                        <View style={ponstyle.offn_views}></View>
                        <View style={ponstyle.offn_view}>
                            <Text style={ponstyle.loc_code_text}>Offence Number</Text>
                            <Text style={ponstyle.loc_code_no}>{Global_Attributes.PonOneBean['formatted']}</Text>
                        </View>

                    </View>

                    <View style={ponstyle.offr_name_view}>
                        <Text style={ponstyle.offr_name}>Officer Name : {Global_Attributes.PonOneBean['officerName']}</Text>
                    </View>
                    ...
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='DATE' underlineColor={'#000000'}
                                value={cDate}
                                onChangeText={(text) => { this.setState({ date: text }) }}
                                onFocus={this.showDatepicker}
                                keyboardType="numeric"
                            ></TextInput>
                              <Image source={require('../../assets/ic_menu_my_calendar.png')} style={{width:35, height:35, position:'absolute', left:140, bottom:6}}/>
                        </View>
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='TIME' underlineColor={'#000000'}
                    ...
                    <View style={ponstyle.flash_view}>
                        ...
                        <View style={ponstyle.scan_btn} >
                            <TouchableOpacity
                                onPress={() => {  this.discoverDeviceHandler(this.state.flash)  }}
                            >
                    ...
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.inputTextStyleThreeView}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='FAMILY*' underlineColor={'#000000'}
                                value={this.state.family}
                                onChangeText={(text) => this.setState({ family: text })}
                            ></TextInput>
                        </View>

                        <View style={ponstyle.inputTextStyleThreeViews}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='GIVEN' underlineColor={'#000000'}
                                value={this.state.given}
                                onChangeText={(text) => this.setState({ given: text })}
                            ></TextInput>
                        </View>
                        <View style={ponstyle.inputTextStyleThreeViewss}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='INITIALS' underlineColor={'#000000'}
                    ...
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='NUMBER AND STREET' underlineColor={'#000000'}
                        ...
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='MUNCIPALITY' underlineColor={'#000000'}
                                value={this.state.muncipality}
                                onChangeText={(text) => this.setState({ muncipality: text })}
                            ></TextInput>
                        </View>
                    </View>
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.inputTextStyleThreeView}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='PO' underlineColor={'#000000'}
                                value={this.state.po}
                                onChangeText={(text) => this.setState({ po: text })}
                            ></TextInput>
                        </View>
                        <View style={ponstyle.inputTextStyleThreeViews}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='PROVINCE' underlineColor={'#000000'}
                        ...
                        <View style={ponstyle.inputTextStyleThreeViewss}>
                            <TextInput style={ponstyle.inputTextStyleThree}
                                label='POSTAL CODE' underlineColor={'#000000'}
                                value={this.state.postl}
                                onChangeText={(text) => this.setState({ postl: text })}
                            ></TextInput>
                        </View>
                    </View>
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='DRIVERS LICENSE NUMBER' underlineColor={'#000000'}
                                labelStyle={{  }}
                                value={this.state.lisenceNumber}
                                onChangeText={(text) => this.setState({ lisenceNumber: text })}
                            ></TextInput>
                        </View>
                        <View style={ponstyle.main_text_views}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='JURIS' value='ON' underlineColor={'#000000'}
                                value={this.state.juris}
                                onChangeText={(text) => this.setState({ juris: text })}
                            ></TextInput>
                        </View>
                    </View>
                    <View style={ponstyle.main_text_view}>
                        <View style={ponstyle.birthView}>
                            <TextInput style={ponstyle.inputTextStyleTwo}
                                label='DATE OF BIRTH' underlineColor={'#000000'}
                                labelStyle={{  }}
                                editable={true}
                                value={bd && bd.getFullYear() + "/" + (bd.getMonth() + 1) + "/" + bd.getDate()}
                                onFocus={() => this.setState({ dobCal: true })}
                                onChangeText={(text) => this.setState({ dob: text })}
                            ></TextInput>
                              <Image source={require('../../assets/ic_menu_my_calendar.png')} style={{width:35, height:35, position:'absolute', left:140, bottom:13}}/>
                        </View>
                        ...
                        <View style={ponstyle.gend_view}>
                            ...
                            <View style={ponstyle.sec_gend_view}>
                                <RadioButton.Group onValueChange={value => this.setState({ gender: value })}
                                    value={this.state.gender}

                                >
                                    <View style={{ flexDirection: 'row' }}>
                                        <RadioButton value="M" color='#11246F' ></RadioButton>
                                        <Text style={{ alignSelf: 'center', color: '#11246F', fontSize: 12 }}>M</Text>
                                        <RadioButton value="F" color='#11246F'></RadioButton>
                                        <Text style={{ alignSelf: 'center', color: '#11246F', fontSize: 12 }}>F</Text>
                                        <RadioButton value="O" color='#11246F' ></RadioButton>
                                        <Text style={{ alignSelf: 'center', color: '#11246F', fontSize: 12 }}>OTHER</Text>
                                    </View>
                                </RadioButton.Group>
                    ...
                    <View style={ponstyle.nxt_btnView}>
                        ...
                        <View style={ponstyle.nxt_btnView2}>
                            <Button mode='contained' style={ponstyle.nxt_btn}
                                // style={{
                    ...
                    <DashboardFooter navigation={navigation} style={{position:'absolute',bottom:0}} />
