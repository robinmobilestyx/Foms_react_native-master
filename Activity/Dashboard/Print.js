    ...
    getUname = async()=>{
        try{
          ...
          if(username != null){
           ...
           this.props.navigation.navigate('NumberVerification',{isVisible:false});  
          }
        }
        catch(error){
          console.log(error);
...
requestPermission = async(permission)=>{
   ...
   addCharge=async()=>{
    ...
    const controller = await new TicketController().ticketApi(paramsBody,this.props,"PonPrint",Global_Attributes.ponTicketApiUrl)
    controller;
    ...
    return (
       ...
       <View style={{flex:5}}>
           ...
           <Pdf
            ...
            <TouchableOpacity style={PdfViewStyle.touchableOpacity1}
            onPress={()=> {this.checkPermission(PERMISSION_TYPE.bluetooth)}}
            >
                <Text style={PdfViewStyle.text} >PRINT</Text>
            </TouchableOpacity>
           { module == "PonInfo"?<TouchableOpacity style={PdfViewStyle.touchableOpacity2}
                onPress={()=> {this.addCharge()}}>
               { module == "PonInfo"?<Text style={PdfViewStyle.text}>ADDITIONAL CHARGE</Text>
