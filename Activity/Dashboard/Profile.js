...
import { SafeAreaView, StyleSheet, View,Image,Text, TouchableOpacity,Platform,BackHandler, Alert } from 'react-native';
...
export default class Profile extends Component{
    ...
    componentDidMount=async()=>{
        ...
        {   
            Alert.alert("Alert!", "OTP Verified. Please set your signature in profile !");
            this.setState({showUpdateBtn:true,editable:true,TouchableOpacityDisable:false,});
        }
    }
...
Profiledata = async()=>{
    ...
    //   console.log("Session Values on login:"+ platoon);
      const phonenumber = await AsyncStorage.getItem('phoneNumber');
    //   console.log("Session Values on login:"+ phonenumber);
      const fname = await AsyncStorage.getItem('fname');
    //   console.log("Session Values on login:"+ fname);
      const lname = await AsyncStorage.getItem('lname');
    //   console.log("Session Values on login:"+ lname);
      const email = await AsyncStorage.getItem('email');
    //   console.log("Session Values on login:"+ email);
      const Sign = await AsyncStorage.getItem('signature');
    //   console.log("Session Values on login:"+ Sign);

      const city = await AsyncStorage.getItem('city');
    //   console.log("Session Values on login:"+ city);
      
    //  const Sign_Path = await AsyncStorage.getItem('Sign_Path');
    //   console.log("Session Values on login:"+ Sign_Path);

        this.setState({
            officername:officername,
            officerno:officerno,platoon:platoon,
  ...
  updateBtn =async()=>{
    ...
    else
    {
        ...
        formData.append("country_code",await AsyncStorage.getItem('country_code'));
        ...
        formData.append("laws_version",await AsyncStorage.getItem('laws_version'));
        ...
        else
        {
            ...
            const signature = {
                uri:Platform.OS === "android" ? "file://"+await AsyncStorage.getItem('Sign_Path') : await AsyncStorage.getItem('Sign_Path').replace("file://", ""),//path of image
                name: await AsyncStorage.getItem('userName')+"_"+timestamp+".png", // name of the image
                type: 'image/jpeg', // image type
              };
            formData.append('signature_img',signature );
        }

        new UpdateUserDataController().update(formData,this.props,"Profile",Global_Attributes.profile);
    }
}
        ...
        <SafeAreaView style={styles.mainContainer}>
            <View style={styles.View}>
                <View style={styles.mainView}>
                    ...
                    <Text style={styles.profile_platoon}>{"Platoon-"+this.state.platoon}</Text>
                    <View style={styles.profile_view}>
                    <Text style={styles.profile_name}>{this.state.officername}</Text>
                    <Text style={styles.profile_email}>{this.state.email}</Text>
                    <Text style={styles.profile_number}>{this.state.phonenumber}</Text>
                    ...
                    <TouchableOpacity
                        ...
                        style={{flex:1}}
                        onPress={()=>{ this.props.navigation.navigate("SignPad");}}>
                    ...
                    {!this.state.showUpdateBtn ?
                        ...
                        >
                        <Text style={{color:'white',alignSelf:'center',fontSize:15,fontWeight:'700'}}>EDIT</Text>
                        </TouchableOpacity>
                        ...
                        >
                            <Text style={{color:'white',alignSelf:'center',fontSize:15,fontWeight:'700'}}>UPDATE</Text>
                        </TouchableOpacity>
