...
import { View, Text,StatusBar,Image,ScrollView,Animated } from 'react-native';
  ...
  this.state={
    ...
    //  let i;
      for (let i= 0; i <faqobjlength; i++) {
        ...
        var tempItem=  (
          <View key={i} style={{marginBottom:3,borderRadius:10,marginLeft:10,marginRight:10}}>
           ...
           <CollapsibleView
           title={<><Text style={{flex:1,padding:5, color: "black", fontSize: 19,}}>{title}</Text>
           <Image source={Downarrow} style={{width:20,height:20,marginRight:0}} />
           </>}
          //  isRTL={true}
          noArrow={true}
